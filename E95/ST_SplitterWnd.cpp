...
int ST_SplitterWnd::Count = 0;
ST_SplitterWnd::ST_SplitterWnd(int nLevel/* = 0*/) :
	m_nHiddenCol(-1), m_nHiddenRow(-1),
...
ST_SplitterWnd* ST_SplitterWnd::AddSubDivision(int nSide,
	...
	if ((nSide >= 0) && (nSide <= 1)) {
		...
		Count++;
		m_pSubSplitterWnd[nSide] = new  ST_SplitterWnd(Count);
...
int ST_SplitterWnd::AddView(int nSide, CRuntimeClass* pViewClass, CCreateContext* pContext) {
	...
	if (CreateView(nRow, nCol, pViewClass, CSize(40, 30), pContext) == 0) return -1;
...
// Note: nSide is always zero at this moment [12/3/2011 XPMUser]
void ST_SplitterWnd::SwitchToView(int nSide, int nViewIX /* = -1 */) {
	...
	CWnd* pWnd0 = m_views[nSide][m_nCurrentView[nSide]];
	...
	if (pWnd0) {
		pWnd0->PostMessage(WM_STDESELECTED, 0, 0);
		Sleep(250);
	}
	if (pWnd) {
		pWnd->PostMessage(WM_STSELECTED, 0, 0);
	}
}