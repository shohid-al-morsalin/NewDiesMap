...
IMPLEMENT_DYNCREATE(CWMapView, CGLView)

BEGIN_MESSAGE_MAP(CWMapView, CGLView)
	ON_WM_RBUTTONUP()
	ON_WM_RBUTTONDOWN()
...
BOOL CWMapView::PreCreateWindow(CREATESTRUCT& cs) {
	return CView::PreCreateWindow(cs);
}

BOOL CWMapView::OnPreparePrinting(CPrintInfo* pInfo) {
	return DoPreparePrinting(pInfo);
}

#ifdef _DEBUG
void CWMapView::AssertValid() const {
	CView::AssertValid();
}

void CWMapView::Dump(CDumpContext& dc) const {
	CView::Dump(dc);
}
#endif

void CWMapView::DrawScene() {
	Point3D pt;
...
void CWMapView::Vertex(float x, float y, float z) {
	int c = 26 * z;
	if (c > 25)  c = 25;
	if (c < 0) ASSERT(0);
...
void CWMapView::SetGraphPoints(vector <Point3D>& RawPoints) {
...
void CWMapView::GenFakeData(vector <Point3D>& RawPoints) {
	Point3D P3D;
...
void CWMapView::GenRealData(CRecipe413* pRcp, std::vector <Point3D>& RawPoints) {
...
void CWMapView::Circle(vector <Point3D>& RawPoints) {
	int idx = 1;
	float r, d;
	inc = 1;
...
void CWMapView::DrawCircle(vector <Point3D>& RawPoints, float r, float dr, int idx) {
	float a;
...
void CWMapView::MakeMashPoint(vector <Point3D>& RawPoints, float x, float y) {
...
void CWMapView::ShowLegend(CRecipe413* pRcp) {
	if (p413App->Global.callFrom3DAutoSave == true) {
		if (!pRcp) return;
		int off = 500;
...
void CWMapView::OnRButtonUp(UINT nFlags, CPoint point) {
	//CMenu menu;
...
void CWMapView::OnWaferviewToolExport() {
...
void CWMapView::OnRButtonDown(UINT nFlags, CPoint point) {
	//	CGLView::OnRButtonDown(nFlags, point);
}

void CWMapView::BuildAxisList() {
	if (p413App->Global.callFrom3DAutoSave == true) {
		double x = 1.1;
...
void CWMapView::PrintString(string str, double x, double y, double z) {
	//	HDC m_hDC = ::GetDC(m_hWnd);
	//	BEGIN_GL
	::glRasterPos3f(x, y, z);
...
void CWMapView::MakeMashPoint(float x, float y, float z) {
	Point3D pt = Point3D(0.9 * x, -0.9 * y, z);
	MashPoints.push_back(pt);
}

void CWMapView::DrawColorBar() {
	glBegin(GL_QUADS);
	int i; for (i = 0; i < 26; i++) {
...
void CWMapView::CreateFont(string fontName, int height) {
	HDC m_hDC = ::GetDC(m_hWnd);
...
void CWMapView::OnInitialUpdate() {
	CGLView::OnInitialUpdate();
}

void CWMapView::printstring(void* font, char* string) {
...
int CWMapView::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
...
void CWMapView::ExportTo(CString exportPath) {
...
void CWMapView::ExportToCImage(CString exportPath) {
	UpdateWindow();
	...
	// Cleanup
	ReleaseDC(pDC);
	HPDF_Page Page = p413App->PdfExporter.GetPage(p413App->PdfExporter.ExtraPage-1);
	float pageHeight = HPDF_Page_GetHeight(Page);
	p413App->PdfExporter.InsertBottomImage(
		Page,                    // HPDF_Page
		pageHeight,                  // float startY (adjust based on your page height)
		exportPath,    // CString title
		_T(""),                  // CString probeName
		image,                   // CImage (will be converted internally)
		_T("3D_IMAGE_VIEW") // CString caption
	);
 }

void CWMapView::OnDraw(CDC* pDC) {
	if (p413App->Global.callFrom3DAutoSave == true) {
