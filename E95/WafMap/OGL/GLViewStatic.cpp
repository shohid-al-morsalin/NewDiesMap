...
IMPLEMENT_DYNCREATE(CGLViewStatic, CView)

CGLViewStatic::CGLViewStatic() {
	m_GridSize = 41;
...
CGLViewStatic::~CGLViewStatic() {
	if (pDIBSurf) delete pDIBSurf;
}

BEGIN_MESSAGE_MAP(CGLViewStatic, CView)
	//{{AFX_MSG_MAP(CGLViewStatic)
	ON_WM_ERASEBKGND()
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_TIMER()
...
/////////////////////////////////////////////////////////////////////////////
// CGLViewStatic drawing

void CGLViewStatic::OnDraw(CDC* pDC) {
...
void CGLViewStatic::OnPrint(CDC* pDC, CPrintInfo* pInfo) {
...
/////////////////////////////////////////////////////////////////////////////
// CGLViewStatic diagnostics

#ifdef _DEBUG
void CGLViewStatic::AssertValid() const {
	CView::AssertValid();
}

void CGLViewStatic::Dump(CDumpContext& dc) const {
	CView::Dump(dc);
}
#endif //_DEBUG

/////////////////////////////////////////////////////////////////////////////
// CGLViewStatic message handlers

void CGLViewStatic::DefaultGraphic() {
}

BOOL CGLViewStatic::PreCreateWindow(CREATESTRUCT& cs) {
	cs.style |= WS_CLIPSIBLINGS | WS_CLIPCHILDREN;
	return CView::PreCreateWindow(cs);
}

void CGLViewStatic::SetLight() {
	...
	float fPos[] =
	{
		(m_LightParam[0] - 50.0f)/**m_fRangeX/100*/,
		(m_LightParam[1] - 50.0f)/**m_fRangeY/100*/,
		(m_LightParam[2] - 50.0f)/**m_fRangeZ/100*/,
		1.f
	};
...
BOOL CGLViewStatic::OnEraseBkgnd(CDC* pDC) {
	return TRUE;
}

int CGLViewStatic::OnCreate(LPCREATESTRUCT lpCreateStruct) {
...
0			// Now is ignored
	...
	if (pDIBSurf) {
		sizeBitmap.cx = 50;
		sizeBitmap.cy = 50;
		pDIBSurf->Create(sizeBitmap.cx, sizeBitmap.cy, (HPALETTE)NULL);
...
void CGLViewStatic::DrawScene() {
}

void CGLViewStatic::OnSize(UINT nType, int cx, int cy) {
...
void CGLViewStatic::OnTimer(UINT nIDEvent) {
	KillTimer(1);
	LimitAngles();
	//====== Increase the angles
	m_AngleX += m_dy;
...
void CGLViewStatic::LimitAngles() {
...
void CGLViewStatic::OnLButtonUp(UINT nFlags, CPoint point) {
...
void CGLViewStatic::OnLButtonDown(UINT nFlags, CPoint point) {
	//====== Stop rotation
	KillTimer(1);
...
void CGLViewStatic::OnMouseMove(UINT nFlags, CPoint point) {
	if (m_bCaptured) {
...
void CGLViewStatic::OnRButtonDown(UINT nFlags, CPoint point) {
	//====== Remember the fact
	m_bRightButton = true;
...
void CGLViewStatic::OnRButtonUp(UINT nFlags, CPoint point) {
	m_bRightButton = m_bCaptured = false;
	ReleaseCapture();
}

void CGLViewStatic::OnOptionFill() {
...
void CGLViewStatic::OnUpdateOptionFill(CCmdUI* pCmdUI) {
	pCmdUI->SetCheck(m_FillMode == GL_FILL);
}

void CGLViewStatic::OnOptionQuad() {
	//	m_bQuad = ! m_bQuad;
...
void CGLViewStatic::OnUpdateOptionQuad(CCmdUI* pCmdUI) {
	//	pCmdUI->SetCheck(m_bQuad==TRUE);
}

void CGLViewStatic::OnOptionBackground() {
...
void CGLViewStatic::OnOptionProperties() {
}

void CGLViewStatic::SetLightParam(short lp, int nPos) {
	m_LightParam[lp] = nPos;
	Invalidate(FALSE);
}

void CGLViewStatic::GetLightParams(int* pPos) {
	int i; for (i = 0; i < 11; i++)
		pPos[i] = m_LightParam[i];
}

void CGLViewStatic::OnOptionSmooth() {
	m_bSmooth = !m_bSmooth;
...
void CGLViewStatic::OnUpdateOptionSmooth(CCmdUI* pCmdUI) {
	pCmdUI->SetCheck(m_bSmooth);
}

BOOL CGLViewStatic::SetDCPixelFormat(HDC hDC, DWORD dwFlags) {
...
UINT CGLViewStatic::GetPropDlgID() {
	return 0;
}

void CGLViewStatic::Cart2Polar(float& r, float& a) {
	float x = r;
	float z = a;
	if (!x && !z) {
		r = 0; a = 0;
...
void CGLViewStatic::Polar2Cart(float& x, float& y) {
	float r = x;
	float a = y * 2 * PI / 360.0f;
...
void CGLViewStatic::OnInitialUpdate() {
...
void CGLViewStatic::ShowLegend(CRecipe413* pRcp) {
}

void CGLViewStatic::CreateFont(string fontName, int height) {
