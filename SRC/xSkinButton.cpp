...
CxSkinButton::CxSkinButton() {
	NewState = OldState = LTOFF;
	bBlinking = FALSE;
	bEnaMouseActivity = FALSE;
...
void CxSkinButton::OnTimer(UINT nIDEvent) {
	if (nIDEvent == IDT_BLINK) {
		KillTimer(IDT_BLINK);
		if (bBlinking) {
			bBlinking = FALSE;
			SetTimer(IDT_BLINK, 300, NULL);
		}
		else {
			bBlinking = TRUE;
...
void CxSkinButton::Blink(BOOL bBlk) {
	if (NewState == LTBLINK) {
		KillTimer(IDT_BLINK);
	}
	if (bBlk) {
