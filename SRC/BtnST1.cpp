...
#define IDT_BLINK	9999

CButtonST1::CButtonST1() {
	//{{AFX_DATA_INIT(CButtonST1)
	//}}AFX_DATA_INIT
...
BEGIN_MESSAGE_MAP(CButtonST1, CButton)
	//{{AFX_MSG_MAP(CButtonST1)
	ON_WM_SETCURSOR()
	ON_WM_KILLFOCUS()
	ON_WM_MOUSEMOVE()
...
void CButtonST1::FreeResources(BOOL bCheckForNULL) {
	if (bCheckForNULL) {
		// Destroy icons
...
void CButtonST1::PreSubclassWindow() {
	UINT nBS;
...
BOOL CButtonST1::PreTranslateMessage(MSG* pMsg) {
	InitToolTip();
...
HBRUSH CButtonST1::CtlColor(CDC* pDC, UINT nCtlColor) {
	return (HBRUSH)::GetStockObject(NULL_BRUSH);
} // End of CtlColor

void CButtonST1::OnSysColorChange() {
...
LRESULT CButtonST1::OnSetStyle(WPARAM wParam, LPARAM lParam) {
...
LRESULT CButtonST1::OnSetCheck(WPARAM wParam, LPARAM lParam) {
...
LRESULT CButtonST1::OnGetCheck(WPARAM wParam, LPARAM lParam) {
	ASSERT(m_bIsCheckBox);
	return GetCheck();
} // End of OnGetCheck

#ifdef	BTNST_USE_BCMENU
LRESULT CButtonST1::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu) {
	LRESULT lResult;
...
#ifdef	BTNST_USE_BCMENU
void CButtonST1::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) {
	BOOL bSetFlag = FALSE;
...
void CButtonST1::OnEnable(BOOL bEnable) {
...
void CButtonST1::OnKillFocus(CWnd* pNewWnd) {
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
} // End of OnKillFocus

void CButtonST1::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) {
	CButton::OnActivate(nState, pWndOther, bMinimized);
	if (nState == WA_INACTIVE)	CancelHover();
} // End of OnActivate

void CButtonST1::OnCancelMode() {
	CButton::OnCancelMode();
	CancelHover();
} // End of OnCancelMode

BOOL CButtonST1::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) {
...
void CButtonST1::OnMouseMove(UINT nFlags, CPoint point) {
	if (!m_bProcessMouseOver) {
		return;
	}
...
LRESULT CButtonST1::OnMouseLeave(WPARAM wParam, LPARAM lParam) {
	CancelHover();
	return 0;
} // End of OnMouseLeave

BOOL CButtonST1::OnClicked() {
	bBlkOn = FALSE;
	KillTimer(IDT_BLINK);
...
void CButtonST1::DrawItem(LPDRAWITEMSTRUCT lpDIS) {
...
void CButtonST1::PaintBk(CDC* pDC) {
...
HBITMAP CButtonST1::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor) {
	HBITMAP		hMask = NULL;
	HDC			hdcSrc = NULL;
	HDC			hdcDest = NULL;
...
//
void CButtonST1::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage) {
	CRect rBtn;
...
void CButtonST1::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled) {
	BYTE		byIndex = 0;
...
void CButtonST1::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled) {
	HDC			hdcBmpMem = NULL;
...
void CButtonST1::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled) {
...
//
HBITMAP CButtonST1::CreateGrayscaleBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans) {
...
//		If the function fails, the return value is NULL.
//
HBITMAP CButtonST1::CreateDarkerBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans) {
	HBITMAP		hGrayBitmap = NULL;
	HDC			hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
	HBITMAP		hOldBmp1 = NULL, hOldBmp2 = NULL;
...
//
HICON CButtonST1::CreateGrayscaleIcon(HICON hIcon) {
...
//		If the function fails, the return value is NULL.
//
HICON CButtonST1::CreateDarkerIcon(HICON hIcon) {
	HICON		hGrayIcon = NULL;
	HDC			hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
	BITMAP		bmp;
	HBITMAP		hOldBmp1 = NULL, hOldBmp2 = NULL;
...
COLORREF CButtonST1::DarkenColor(COLORREF crColor, double dFactor) {
...
//
DWORD CButtonST1::SetIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut, int nCxDesiredOut, int nCyDesiredOut) {
	HICON		hIconIn = NULL;
	HICON		hIconOut = NULL;
...
//			Failed loading the specified resource.
//
DWORD CButtonST1::SetIcon(int nIconIn, int nIconOut) {
...
//			Failed loading the specified resource.
//
DWORD CButtonST1::SetIcon(HICON hIconIn, HICON hIconOut) {
	BOOL		bRetValue;
	ICONINFO	ii;
...
//
DWORD CButtonST1::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut) {
...
//			Failed creating mask bitmap.
//
DWORD CButtonST1::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut) {
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
...
//				By default, CButtonST1 buttons are flat.
//		[IN]	bRepaint
...
//			Function executed successfully.
//
DWORD CButtonST1::SetFlat(BOOL bFlat, BOOL bRepaint) {
	m_bIsFlat = bFlat;
...
//				By default, CButtonST1 buttons have ST_ALIGN_HORIZ alignment.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CButtonST1::SetAlign(BYTE byAlign, BOOL bRepaint) {
	switch (byAlign) {
	case ST_ALIGN_HORIZ:
	case ST_ALIGN_HORIZ_RIGHT:
	case ST_ALIGN_VERT:
...
//				By default, CButtonST1 buttons have BTNST_PRESSED_LEFTRIGHT style.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CButtonST1::SetPressedStyle(BYTE byStyle, BOOL bRepaint) {
	switch (byStyle) {
...
//			Function executed successfully.
//
DWORD CButtonST1::SetCheck(int nCheck, BOOL bRepaint) {
	if (m_bIsCheckBox) {
...
//			Function executed successfully.
//
DWORD CButtonST1::SetDefaultColors(BOOL bRepaint) {
...
//			Invalid color index.
//
DWORD CButtonST1::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint) {
...
//			Invalid color index.
//
DWORD CButtonST1::GetColor(BYTE byColorIndex, COLORREF* crpColor) {
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//
DWORD CButtonST1::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint) {
	BYTE	byRed = 0;
	BYTE	byGreen = 0;
	BYTE	byBlue = 0;
...
//			Function executed successfully.
//
DWORD CButtonST1::SetAlwaysTrack(BOOL bAlwaysTrack) {
...
//			Failed loading the specified resource.
//
DWORD CButtonST1::SetBtnCursor(int nCursorId, BOOL bRepaint) {
	HINSTANCE	hInstResource = NULL;
...
//			Function executed successfully.
//
DWORD CButtonST1::DrawBorder(BOOL bDrawBorder, BOOL bRepaint) {
...
//			Function executed successfully.
//
DWORD CButtonST1::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint) {
...
//
void CButtonST1::SetTooltipText(int nText, BOOL bActivate) {
	CString sText;
...
//				If TRUE the tooltip will be created active.
//
void CButtonST1::SetTooltipText(LPCTSTR lpszText, BOOL bActivate) {
...
//
void CButtonST1::ActivateTooltip(BOOL bActivate) {
...
//			Function executed successfully.
//
DWORD CButtonST1::EnableBalloonTooltip() {
...
//				If TRUE the control will be repainted.
//
void CButtonST1::DrawTransparent(BOOL bRepaint) {
...
DWORD CButtonST1::SetBk(CDC* pDC) {
...
//			Function executed successfully.
//
DWORD CButtonST1::SetURL(LPCTSTR lpszURL) {
...
//
#ifndef	BTNST_USE_BCMENU
DWORD CButtonST1::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint) {
	HINSTANCE	hInstResource = NULL;
...
//
#ifdef	BTNST_USE_BCMENU
DWORD CButtonST1::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint) {
	BOOL	bRetValue = FALSE;
...
//			Function executed successfully.
//
DWORD CButtonST1::SetMenuCallback(HWND hWnd, UINT nMessage, LPARAM lParam) {
...
//
#ifdef	BTNST_USE_SOUND
DWORD CButtonST1::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync) {
...
// This is a virtual function that can be rewritten in CButtonST1-derived classes
...
//			Function executed successfully.
//
DWORD CButtonST1::OnDrawBackground(CDC* pDC, CRect* pRect) {
	COLORREF	crColor;
...
//			Function executed successfully.
//
DWORD CButtonST1::OnDrawBorder(CDC* pDC, CRect* pRect) {
	// Draw pressed button
	if (m_bIsPressed) {
		if (m_bIsFlat) {
			if (m_bDrawBorder) {
...
void CButtonST1::OnTimer(UINT nIDEvent) {
	if (nIDEvent == IDT_BLINK) {
		KillTimer(IDT_BLINK);
		if (bBlkOn) {
			bBlkOn = FALSE;
			SetTimer(IDT_BLINK, 500, NULL);
		}
		else {
			bBlkOn = TRUE;
			SetTimer(IDT_BLINK, 1000, NULL);
		}
		Invalidate();
	}
}

void CButtonST1::SetBlink(BOOL bBK) {
	if (bBK) {
		SetTimer(IDT_BLINK, 1000, NULL);
	}
	else {
