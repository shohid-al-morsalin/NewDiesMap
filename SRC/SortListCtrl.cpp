...
int CSortListCtrl::AddItem(LPCTSTR pszText, ...) {
	...
	arrpsz[0] = new TCHAR[lstrlen(pszText) + 16];
	...
	for (int iColumn = 1; iColumn < m_iNumColumns; iColumn++) {
		...
		arrpsz[iColumn] = new TCHAR[lstrlen(pszText) + 16];
...
void CSortListCtrl::SaveColumnInfo() {
	if (m_iNumColumns < 1) {
		// LYF Not yet initialize so simply end [4/22/2006]
		return;
	}
	//	ASSERT( m_iNumColumns > 0 );
...
BOOL CSortListCtrl::SetItemText(int nItem, int nSubItem, LPCTSTR lpszText) {
	...
	delete[] pszText;
	pszText = new TCHAR[lstrlen(lpszText) + 16];
...
int CSortListCtrl::GetNumColumn() {
