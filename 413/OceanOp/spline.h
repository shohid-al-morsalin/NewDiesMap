...
namespace tk {
	...
	public:
		...
		virtual ~band_matrix() {};                            // destructor
	...
	public:
		...
		spline();
		virtual ~spline() {
			if (pX) delete[]pX; pX = nullptr;
			if (pY) delete[]pY; pY = nullptr;
		};
		...
		//void set_points(float* y, int sz, bool cubic_spline = true);
		void set_points(double* x, double* y, int sz, bool cubic_spline = true);
		int FindIdx(double x) const;
